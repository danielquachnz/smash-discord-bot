...
public class MessageCreatedEventService {
    ...
    public Mono<Message> process(MessageCreateEvent event){
        ...
        return executeCommand(messageChannel, messageComponents);
    }

    private Mono<Message> executeCommand(Mono<MessageChannel> messageChannel, List<String> messageComponents) {
        ...
        try {
            return messageCreatedEventMapperService.getCommandService(command)
                .processMessage(messageChannel, params);
        } catch (final HttpException e) {
            LOG.info(String.format("Encountered HttpException of type %s: %s", e.getClass().getSimpleName(), e.getMessage()));
            return EmbeddedMessageHelper.createMessage(messageChannel, e.getMessage());
